...
public class StringButton extends Button {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Bllry.FONT_MANAGER.drawTextWithShadow(context, displayText, getX() + getTextPadding() + 1, getY() + 2, selecting ? ClickGuiScreen.getButtonColor(getY(), 255) : Color.WHITE);
    }
        ...
        if (ctrl) {
            if (keyCode == GLFW.GLFW_KEY_V) {
                try {
                    ...
                    Bllry.LOGGER.error("{}: Failed to process clipboard paste", exception.getClass().getName(), exception);
                }
                return;
            }
            ...
            if (keyCode == GLFW.GLFW_KEY_C && selecting) {
                try {
                    ...
                    Bllry.LOGGER.error("{}: Failed to process clipboard change", exception.getClass().getName(), exception);
                }
                return;
            }
