...
public class ModuleButton extends Button {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        if(this.isHovering(mouseX, mouseY) && Bllry.CLICK_GUI.getDescriptionFrame().getDescription().isEmpty()) Bllry.CLICK_GUI.getDescriptionFrame().setDescription(this.getDescription());
        ...
        Bllry.FONT_MANAGER.drawTextWithShadow(context, (module.isToggled() ? "" : Formatting.GRAY ) + module.getName(), getX() + getTextPadding(), getY() + 2, Color.WHITE);

        if(open) {
            for(Button button : buttons) {
                if(!button.isVisible()) continue;
                button.render(context, mouseX, mouseY, delta);
                if(button.isHovering(mouseX, mouseY) && Bllry.CLICK_GUI.getDescriptionFrame().getDescription().isEmpty()) Bllry.CLICK_GUI.getDescriptionFrame().setDescription(button.getDescription());
            }
        }
    }
