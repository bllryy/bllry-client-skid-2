...
import me.lily.bllry.mixins.accessors.EntityAccessor;
...
import me.lily.bllry.utils.rotations.Rotation;
...
public class RotationManager implements IMinecraft {
    ...
    public void onUpdateVelocity(UpdateVelocityEvent event) {
        ...
        if (!Bllry.MODULE_MANAGER.getModule(RotationsModule.class).movementFix.getValue()) return;
        if (rotation == null) return;
    ...
    public void packetRotate(float yaw, float pitch) {
        ...
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(Bllry.POSITION_MANAGER.getServerX(), Bllry.POSITION_MANAGER.getServerY(), Bllry.POSITION_MANAGER.getServerZ(), yaw, pitch, Bllry.POSITION_MANAGER.isServerOnGround(), mc.player.horizontalCollision));
    }
