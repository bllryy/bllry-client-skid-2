...
import me.lily.bllry.utils.miscellaneous.RenderPosition;
...
public class RenderManager implements IMinecraft {
    ...
    public void onRenderWorld$placePositions(RenderWorldEvent event) {
        ...
        RendersModule module = Bllry.MODULE_MANAGER.getModule(RendersModule.class);
    ...
    public void onRenderWorld$autoCrystal(RenderWorldEvent event) {
        ...
        AutoCrystalModule autoCrystalModule = Bllry.MODULE_MANAGER.getModule(AutoCrystalModule.class);
    ...
    public void onRenderWorld$autoCrystalExtra(RenderWorldEvent.Post event) {
        ...
        AutoCrystalModule module = Bllry.MODULE_MANAGER.getModule(AutoCrystalModule.class);
        ...
        if (module.icon.getValue()) {
            ...
            if (module.renderDamage.getValue()) {
                Renderer2D.renderTexture(matrices, -5.5f, -8.5f, 5.5f, 2.5f, Identifier.of(Bllry.MOD_ID, "textures/crystal.png"), Color.WHITE);

                matrices.push();
                matrices.scale(0.45f, 0.45f, 0.45f);

                String text = module.getCalculationDamage();
                Bllry.FONT_MANAGER.drawTextWithShadow(matrices, text, -Bllry.FONT_MANAGER.getWidth(text) / 2 - 1, 7, mc.getBufferBuilders().getEntityVertexConsumers(), Color.WHITE);

                matrices.pop();
            } else {
                Renderer2D.renderTexture(matrices, -6.5f, -6.5f, 6.5f, 6.5f, Identifier.of(Bllry.MOD_ID, "textures/crystal.png"), Color.WHITE);
            }
            ...
            if (module.renderDamage.getValue()) {
                ...
                String text = module.getCalculationDamage();
                Bllry.FONT_MANAGER.drawTextWithShadow(matrices, text, -Bllry.FONT_MANAGER.getWidth(text) / 2, -Bllry.FONT_MANAGER.getHeight() / 2, mc.getBufferBuilders().getEntityVertexConsumers(), Color.WHITE);
    ...
    public void setRenderPosition(BlockPos position) {
        if (!Bllry.MODULE_MANAGER.getModule(AutoCrystalModule.class).isToggled()) position = null;
