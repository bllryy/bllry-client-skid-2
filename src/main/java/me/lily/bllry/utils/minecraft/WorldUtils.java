...
import me.lily.bllry.modules.impl.core.RotationsModule;
...
public class WorldUtils implements IMinecraft {
    ...
    public static void placeBlock(BlockPos position, Direction direction, Hand hand, Runnable runnable, boolean rotate, boolean crystalDestruction, boolean render) {
        ...
        float prevYaw = Bllry.ROTATION_MANAGER.getServerYaw();
        float prevPitch = Bllry.ROTATION_MANAGER.getServerPitch();

        if (rotate) Bllry.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(vec3d.getX(), vec3d.getY(), vec3d.getZ()));
        ...
        if (rotate && Bllry.MODULE_MANAGER.getModule(RotationsModule.class).snapBack.getValue()) Bllry.ROTATION_MANAGER.packetRotate(prevYaw, prevPitch);

        Bllry.WORLD_MANAGER.getPlaceTimer().reset();
        ...
        if (render) {
            ...
            if(!Bllry.RENDER_MANAGER.renderPositions.contains(renderPosition)) Bllry.RENDER_MANAGER.renderPositions.add(renderPosition);
        }
    }
