package me.lily.bllry.utils.font;
...
public class FontRenderer implements Closeable, IMinecraft {
    ...
    public void drawText(MatrixStack matrices, OrderedText text, float x, float y, int color, boolean dropShadow) {
        ...
        if (Bllry.MODULE_MANAGER != null && Bllry.MODULE_MANAGER.getModule(FontModule.class).isToggled()) {
            x += Bllry.MODULE_MANAGER.getModule(FontModule.class).xOffset.getValue().floatValue();
            y += Bllry.MODULE_MANAGER.getModule(FontModule.class).yOffset.getValue().floatValue();
        }
