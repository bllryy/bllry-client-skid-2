package me.lily.bllry.utils.text;

import me.lily.bllry.Bllry;
import me.lily.bllry.mixins.accessors.StyleAccessor;
import me.lily.bllry.mixins.accessors.TextColorAccessor;
...
public class FormattingUtils {
    ...
    public static List<String> wrapText(String text, int width) {
        ...
        for(String word : words) {
            if(Bllry.FONT_MANAGER.getWidth(current) + Bllry.FONT_MANAGER.getWidth(word) <= width) {
                current += word + " ";
            } else {
        ...
        if(Bllry.FONT_MANAGER.getWidth(current) > 0) wrappedText.add(current);
