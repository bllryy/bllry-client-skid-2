...
import me.lily.bllry.mixins.accessors.WorldRendererAccessor;
...
public class Renderer3D implements IMinecraft {
    ...
    public static void renderText(MatrixStack matrices, String text, double x, double y, double z, float scaling, boolean background, Color color) {
        ...
        if (background) Renderer2D.renderQuad(matrices, -Bllry.FONT_MANAGER.getWidth(text) / 2.0f - 2, -Bllry.FONT_MANAGER.getHeight() - 2, Bllry.FONT_MANAGER.getWidth(text) / 2.0f + 2, 1, new Color(0, 0, 0, 100));
        Bllry.FONT_MANAGER.drawTextWithShadow(matrices, text, -Bllry.FONT_MANAGER.getWidth(text) / 2, -Bllry.FONT_MANAGER.getHeight(), vertexConsumers, color);
