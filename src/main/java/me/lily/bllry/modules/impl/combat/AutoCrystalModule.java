...
import me.lily.bllry.modules.impl.player.SpeedMineModule;
import me.lily.bllry.modules.impl.player.ThrowXPModule;
//import me.aidan.sydney.settings.impl.*;
import me.lily.bllry.settings.impl.*;
import me.lily.bllry.utils.color.ColorUtils;
import me.lily.bllry.utils.minecraft.DamageUtils;
...
import me.lily.bllry.utils.system.Counter;
...
public class AutoCrystalModule extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        attackedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > Bllry.SERVER_MANAGER.getPing() * 2L);
        placedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > Bllry.SERVER_MANAGER.getPing() * 2L + (20 - attackSpeed.getValue().floatValue()) * 50L);
        countedCrystals.entrySet().removeIf(entry -> System.currentTimeMillis() - entry.getValue() > Bllry.SERVER_MANAGER.getPing() * 2L);
        ...
        Runnable runnable = () -> {
            ...
            if (blockDestruction.getValue() && asynchronous.getValue()) {
                SpeedMineModule module = Bllry.MODULE_MANAGER.getModule(SpeedMineModule.class);
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Packet")) Bllry.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)));
        if (rotate.getValue().equalsIgnoreCase("Normal")) Bllry.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(crystal.getBlockPos(), 0)), Bllry.ROTATION_MANAGER.getModulePriority(this));
    ...
    public void onDestroyBlock(DestroyBlockEvent event) {
        ...
        if (mineTarget == null || mineTarget.getPosition() == null) {
            Bllry.RENDER_MANAGER.setRenderPosition(null);
            return;
        }

        BlockPos position = mineTarget.getPosition();
        Bllry.RENDER_MANAGER.setRenderPosition(position);
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) Bllry.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), this, Bllry.ROTATION_MANAGER.getModulePriority(this) + 1);
        if (!rotate.getValue().equalsIgnoreCase("None")) Bllry.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));
        ...
        for (Entity entity : mc.world.getOtherEntities(null, new Box(position.up())).stream().filter(entity -> entity instanceof EndCrystalEntity).toList()) {
            if (!rotate.getValue().equalsIgnoreCase("None")) Bllry.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(entity));
    ...
    private void placeCrystals(boolean sequential) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) Bllry.ROTATION_MANAGER.rotate(calculateRotations(Vec3d.ofCenter(position, 1)), Bllry.ROTATION_MANAGER.getModulePriority(this));
        ...
        placeRunnable = () -> {
            ...
            if (rotate.getValue().equalsIgnoreCase("Packet")) Bllry.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(Vec3d.ofCenter(position, 1)));
            ...
            if (godSync.getValue()) {
                boolean flag = !antiKick.getValue() || !(mc.player.getMainHandStack().getItem() instanceof ExperienceBottleItem) && !(mc.player.getOffHandStack().getItem() instanceof ExperienceBottleItem) && !Bllry.MODULE_MANAGER.getModule(ThrowXPModule.class).isToggled();
    ...
    private EndCrystalEntity calculateCrystals() {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            if (!Bllry.MODULE_MANAGER.getModule(SuicideModule.class).isToggled()) {
    ...
    private PlaceTarget calculatePlacements(BlockPos exception) {
        ...
        for (int i = 0; i < Bllry.WORLD_MANAGER.getRadius(Math.max(placeRange.getValue().doubleValue(), placeWallsRange.getValue().doubleValue())); i++) {
    ...
    private float[] calculateRotations(Vec3d vec3d) {
        ...
        if (yawStep.getValue()) {
            ...
            float difference = Bllry.ROTATION_MANAGER.getServerYaw() - rotations[0];
            ...
            if (Math.abs(difference) > yawStepThreshold.getValue().floatValue()) yaw = Bllry.ROTATION_MANAGER.getServerYaw() + deltaYaw;
