        ...
        Runnable runnable = () -> {
            ...
            if (autoSwitch.getValue().equalsIgnoreCase("None") && mc.player.getMainHandStack().getItem() != Items.COBWEB) {
                if (itemDisable.getValue()) {
                    Bllry.CHAT_MANAGER.tagged("You are currently not holding any cobwebs.", getName());
                    setToggled(false);
                }
            ...
            if (slot == -1) {
                if (itemDisable.getValue()) {
                    Bllry.CHAT_MANAGER.tagged("No cobwebs could be found in your hotbar.", getName());
                    setToggled(false);
                }
            ...
            if(rotate.getValue() && airPlace.getValue()) Bllry.ROTATION_MANAGER.rotate(RotationUtils.getRotations(position.toCenterPos()), this);
