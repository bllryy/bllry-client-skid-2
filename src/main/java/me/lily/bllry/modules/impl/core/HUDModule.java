...
import me.lily.bllry.utils.animations.Animation;
...
public class HUDModule extends Module {
    ...
    public StringSetting watermarkText = new StringSetting("WatermarkText", "Text", "The client name that will be rendered.", new CategorySetting.Visibility(watermarkCategory), Bllry.MOD_NAME);
    ...
    public void onTick(TickEvent event) {
        ...
        if (moduleList.getValue()) {
            Comparator<Module> widthComparator = Comparator.comparingInt(m -> -Bllry.FONT_MANAGER.getWidth(getModuleText(m)));
            Comparator<Module> alphabeticalComparator = Comparator.comparing(Module::getName);

            List<ModuleEntry> entries = new ArrayList<>();
            List<Module> modules = Bllry.MODULE_MANAGER.getModules().stream()
        ...
        if (playerRadar.getValue()) {
            ...
            for (PlayerEntity player : players) {
                ...
                int pops = Bllry.WORLD_MANAGER.getPoppedTotems().getOrDefault(player.getUuid(), 0);
        ...
        if (potions.getValue()) {
            Comparator<StatusEffectInstance> widthComparator = Comparator.comparingInt(e -> -Bllry.FONT_MANAGER.getWidth(e.getEffectType().value().getName().getString() + " " + (e.getAmplifier() + 1)));
    ...
    public void renderWatermark(RenderOverlayEvent event) {
        ...
        if (watermark.getValue()) {
            String text = watermarkText.getValue() + (watermarkVersion.getValue() ? (watermarkSync.getValue() ? "" : inversion.getValue() ? Formatting.GRAY : Formatting.WHITE) + " " + Bllry.MOD_VERSION + (watermarkMinecraftVersion.getValue() ? "-mc" + Bllry.MINECRAFT_VERSION : "") + (watermarkRevision.getValue() ? "+" + Bllry.GIT_REVISION + "." + Bllry.GIT_HASH : "") : "");
            drawText(event.getContext(), text, 2, 2);
        }

        if(uptime.getValue()) {
            String[] hms = FormattingUtils.formatSeconds((System.currentTimeMillis() - Bllry.UPTIME)/1000);
            drawText(event.getContext(), "Uptime " + Formatting.WHITE + hms[0] + ":" + hms[1] + ":" + hms[2], 2, 2 + (watermark.getValue() ? Bllry.FONT_MANAGER.getHeight() : 0), informationSync.getValue() ? null : new Color(170, 170, 170));
        }
        ...
        if (welcomer.getValue()) {
            ...
            drawText(event.getContext(), text, mc.getWindow().getScaledWidth() / 2.0f - Bllry.FONT_MANAGER.getWidth(text) / 2.0f, 6);
        }
    }
    ...
    public void renderModuleList(RenderOverlayEvent event) {
        ...
        for (ModuleEntry entry : moduleEntries) {
            float x = mc.getWindow().getScaledWidth() - (entry.module().getAnimationOffset().get(entry.module().isToggled() ? Bllry.FONT_MANAGER.getWidth(entry.text()) + 2 : 0));
            float y = 2 + potionOffset + (index * Bllry.FONT_MANAGER.getHeight());
    ...
    public void renderPlayerRadar(RenderOverlayEvent event) {
        ...
        for (PlayerEntry entry : playerEntries) {
            if (entry.headTexture() != null) PlayerSkinDrawer.draw(event.getContext(), entry.headTexture(), 2, 1 + (Bllry.FONT_MANAGER.getHeight() * 2) + ((Bllry.FONT_MANAGER.getHeight() + 1) * offset), Bllry.FONT_MANAGER.getHeight(), true, false, Color.WHITE.getRGB());
            drawText(event.getContext(), entry.text(), 2 + (entry.headTexture() != null ? Bllry.FONT_MANAGER.getHeight() + 2 : 0), 2 + (Bllry.FONT_MANAGER.getHeight() * 2) + ((Bllry.FONT_MANAGER.getHeight() + 1) * offset), Bllry.FRIEND_MANAGER.contains(entry.player().getName().getString()) ? Bllry.FRIEND_MANAGER.getDefaultFriendColor() : null);
    ...
    public void renderInformation(RenderOverlayEvent event) {
        ...
        if (health.getValue()) {
            ...
            Bllry.FONT_MANAGER.drawTextWithOutline(event.getContext(), text, mc.getWindow().getScaledWidth() / 2 - Bllry.FONT_MANAGER.getWidth(text) / 2, mc.getWindow().getScaledHeight() / 2 + 16, Color.WHITE, Color.BLACK);
        }
        ...
        if (potions.getValue()) {
            for (PotionEntry entry : potionEntries) {
                if (entry.sprite() != null) {
                    matrices.push();
                    matrices.translate(mc.getWindow().getScaledWidth() - 2 - Bllry.FONT_MANAGER.getWidth(entry.text()) - Bllry.FONT_MANAGER.getHeight() - 2, mc.getWindow().getScaledHeight() - chatOffset - 2 - Bllry.FONT_MANAGER.getHeight() - (Bllry.FONT_MANAGER.getHeight() * offset) - 1, 0);
                    event.getContext().drawSpriteStretched(RenderLayer::getGuiTextured, entry.sprite(), 0, 0, Bllry.FONT_MANAGER.getHeight(), Bllry.FONT_MANAGER.getHeight());
                    matrices.pop();
                }

                drawText(event.getContext(), entry.text(), mc.getWindow().getScaledWidth() - 2 - Bllry.FONT_MANAGER.getWidth(entry.text()), mc.getWindow().getScaledHeight() - chatOffset - 2 - Bllry.FONT_MANAGER.getHeight() - (Bllry.FONT_MANAGER.getHeight() * offset), potionColor.getValue().equals("Client") && colorMode.getValue().equals("Rainbow") && rainbowMode.getValue().equals("Horizontal"), entry.color());
                offset++;
            }
        }
        ...
        if (ping.getValue()) informationEntries.add(getPrimary() + "Ping " + getSecondary() + Bllry.SERVER_MANAGER.getPing() + "ms");
        if (fps.getValue()) informationEntries.add(getPrimary() + "FPS " + getSecondary() + Bllry.RENDER_MANAGER.getFps());
        ...
        if (serverBrand.getValue()) informationEntries.add(getPrimary() + "Brand " + getSecondary() + Bllry.SERVER_MANAGER.getServerBrand());

        float tickRate = Bllry.SERVER_MANAGER.getTickRate();
        ...
        if (!informationEntries.isEmpty()) {
            informationEntries.sort(Comparator.comparingInt(Bllry.FONT_MANAGER::getWidth).reversed());
            for (String text : informationEntries) {
                if (text.startsWith("Durability")) {
                    if (mc.player.getMainHandStack().isDamageable()) {
                        ...
                        drawText(event.getContext(), getPrimary() + "Durability ", mc.getWindow().getScaledWidth() - 2 - Bllry.FONT_MANAGER.getWidth("Durability ") - Bllry.FONT_MANAGER.getWidth(s), mc.getWindow().getScaledHeight() - chatOffset - 2 - Bllry.FONT_MANAGER.getHeight() + (offset * -Bllry.FONT_MANAGER.getHeight()), informationSync.getValue() ? null : new Color(170, 170, 170));
                        drawText(event.getContext(), s, mc.getWindow().getScaledWidth() - 2 - Bllry.FONT_MANAGER.getWidth(s), mc.getWindow().getScaledHeight() - chatOffset - 2 - Bllry.FONT_MANAGER.getHeight() + (offset * -Bllry.FONT_MANAGER.getHeight()), false, new Color(1.0f - ((maxDamage - damage) / (float) maxDamage), (maxDamage - damage) / (float) maxDamage, 0));
                        offset++;
                    }
                } else {
                    drawText(event.getContext(), text, mc.getWindow().getScaledWidth() - 2 - Bllry.FONT_MANAGER.getWidth(text), mc.getWindow().getScaledHeight() - chatOffset - 2 - Bllry.FONT_MANAGER.getHeight() + (offset * -Bllry.FONT_MANAGER.getHeight()), informationSync.getValue() ? null : new Color(170, 170, 170));
                    offset++;
                }
            }
        }
    ...
    public void renderCoordinates(RenderOverlayEvent event) {
        ...
        if (coordinates.getValue())  {
            ...
            drawText(event.getContext(), text, 2, mc.getWindow().getScaledHeight() - chatOffset - offset - Bllry.FONT_MANAGER.getHeight() - 2);
            offset += Bllry.FONT_MANAGER.getHeight();
        }
        ...
        if (direction.getValue()) {
            ...
            drawText(event.getContext(), text, 2, mc.getWindow().getScaledHeight() - chatOffset - offset - Bllry.FONT_MANAGER.getHeight() - 2, positionSync.getValue() ? null : Color.WHITE);
        }
    }
    ...
    private void drawModuleText(Module module, DrawContext context, String text, float x, float y) {
        ...
        if (moduleColorMode.getValue().equalsIgnoreCase("Rainbow")) {
            long index = ((long) y / Bllry.FONT_MANAGER.getHeight()) * (rainbowOffset.getValue().longValue() * 10L);
    ...
    public void drawText(DrawContext context, String text, float x, float y, boolean rainbow, Color color) {
        ...
        if (rainbow)  {
            Bllry.FONT_MANAGER.drawRainbowString(context, text, 0, 0, rainbowOffset.getValue().longValue() * 5L);
        } else {
            Bllry.FONT_MANAGER.drawTextWithShadow(context, text, 0, 0, color);
        }
    ...
    private Color getHudColor(float offset) {
        if (colorMode.getValue().equalsIgnoreCase("Rainbow")) {
            long index = ((long) offset / Bllry.FONT_MANAGER.getHeight()) * (rainbowOffset.getValue().longValue() * 10L);
            return ColorUtils.getOffsetRainbow(index);
        } else if (colorMode.getValue().equalsIgnoreCase("Wave")) {
            long index = ((long) offset / Bllry.FONT_MANAGER.getHeight()) * (rainbowOffset.getValue().longValue() * 20L);
