...
import me.lily.bllry.events.impl.UpdateMovementEvent;
...
@RegisterModule(name = "KillAura", description = "Automatically attacks entities if they meet specified requirements.", category = Module.Category.COMBAT)
public class KillAuraModule extends Module {
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        Iterable<Entity> entities = mc.world.getOtherEntities(mc.player, PositionUtils.getRadius(mc.player, Math.max(range.getValue().doubleValue(), wallsRange.getValue().doubleValue()) + 1.0), entity ->
                entity.isAlive()
                        ...
                        && (friends.getValue() || !Bllry.FRIEND_MANAGER.contains(entity.getName().getString()))
        ...
        if (rotate.getValue().equalsIgnoreCase("Hold")) Bllry.ROTATION_MANAGER.rotate(RotationUtils.getRotations(target), this);
        ...
        if (rotate.getValue().equalsIgnoreCase("Normal")) Bllry.ROTATION_MANAGER.rotate(RotationUtils.getRotations(target), this);
    ...
    public void onUpdateMovement$POST(UpdateMovementEvent.Post event) {
        ...
        if (rotate.getValue().equalsIgnoreCase("Packet")) Bllry.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(target));
