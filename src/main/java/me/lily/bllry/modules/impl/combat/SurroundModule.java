...
import me.lily.bllry.modules.impl.movement.StepModule;
...
public class SurroundModule extends Module {
    ...
    public void onEnable() {
        ...
        if(stepToggle.getValue() && Bllry.MODULE_MANAGER.getModule(StepModule.class).isToggled()) Bllry.MODULE_MANAGER.getModule(StepModule.class).setToggled(false);
        if(speedToggle.getValue() && Bllry.MODULE_MANAGER.getModule(SpeedModule.class).isToggled()) Bllry.MODULE_MANAGER.getModule(SpeedModule.class).setToggled(false);
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (mc.player == null || mc.world == null) return;
        if (jumpDisable.getValue() && (mc.player.fallDistance > 2.0f || ((Bllry.MODULE_MANAGER.getModule(StepModule.class).isToggled() || Bllry.MODULE_MANAGER.getModule(SpeedModule.class).isToggled()) && (lastPosition == null || lastPosition.getY() != PositionUtils.getFlooredPosition(mc.player).getY())))) {
            setToggled(false);
            return;
        }
        ...
        if (autoSwitch.getValue().equalsIgnoreCase("None") && !(mc.player.getMainHandStack().getItem() instanceof BlockItem)) {
            if (itemDisable.getValue()) {
                Bllry.CHAT_MANAGER.tagged("You are currently not holding any blocks.", getName());
                setToggled(false);
            }
        ...
        if (slot == -1) {
            if (itemDisable.getValue()) {
                Bllry.CHAT_MANAGER.tagged("No blocks could be found in your hotbar.", getName());
                setToggled(false);
            }
        ...
        HitboxDesyncModule module = Bllry.MODULE_MANAGER.getModule(HitboxDesyncModule.class);
