...
public class AutoTotemModule extends Module {
    ...
    public void onPlayerPop(PlayerPopEvent event) {
        if (event.getPlayer() == mc.player && !Bllry.MODULE_MANAGER.getModule(SuicideModule.class).isToggled()) {
            ticks = 0;
        }
    }
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        if (item == Items.TOTEM_OF_UNDYING && Bllry.MODULE_MANAGER.getModule(SuicideModule.class).isToggled() && Bllry.MODULE_MANAGER.getModule(SuicideModule.class).offhandOverride.getValue()) {
    ...
    private boolean needsTotem() {
        if (Bllry.MODULE_MANAGER.getModule(SuicideModule.class).isToggled() && Bllry.MODULE_MANAGER.getModule(SuicideModule.class).offhandOverride.getValue()) return false;
        ...
        return antiMace.getValue() && (mc.world.getPlayers().stream().anyMatch(entity -> entity != mc.player && !Bllry.FRIEND_MANAGER.contains(entity.getName().getString()) && mc.player.squaredDistanceTo(entity) <= MathHelper.square(maceRange.getValue().floatValue()) && entity.fallDistance >= 1.5 && entity.getMainHandStack().getItem().equals(Items.MACE)));
    }
