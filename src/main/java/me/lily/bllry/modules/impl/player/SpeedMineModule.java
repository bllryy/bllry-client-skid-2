    ...
    public class Action {
        ...
        public boolean process() {
            ...
            if (mining) {
                ...
                float delta = WorldUtils.getMineSpeed(this.state, slot) / Bllry.WORLD_MANAGER.getTimerMultiplier();
                ...
                if (rotate.getValue().equalsIgnoreCase("Normal") && progress + (delta * 2) >= getSpeed()) {
                    Bllry.ROTATION_MANAGER.rotate(RotationUtils.getRotations(WorldUtils.getHitVector(position, direction)), Bllry.ROTATION_MANAGER.getModulePriority(Bllry.MODULE_MANAGER.getModule(SpeedMineModule.class)));
                }
                ...
                if (progress >= getSpeed() && !state.isReplaceable() && (whileEating.getValue() || !mc.player.isUsingItem())) {
                    if (!instantMine || instantTimer.hasTimeElapsed(instantDelay.getValue().longValue() * 50L)) {
                        Bllry.EVENT_HANDLER.post(new DestroyBlockEvent(position));

                        if (rotate.getValue().equalsIgnoreCase("Packet")) Bllry.ROTATION_MANAGER.packetRotate(RotationUtils.getRotations(WorldUtils.getHitVector(position, direction)));
