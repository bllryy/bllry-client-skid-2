...
import me.lily.bllry.mixins.accessors.ItemRenderStateAccessor;
import me.lily.bllry.mixins.accessors.ItemRendererAccessor;
import me.lily.bllry.mixins.accessors.LayerRenderStateAccessor;
import me.lily.bllry.modules.Module;
import me.lily.bllry.modules.RegisterModule;
import me.lily.bllry.modules.impl.miscellaneous.FakePlayerModule;
...
public class NameTagsModule extends Module {
    ...
    public void onRenderWorld(RenderWorldEvent.Post event) {
        ...
        for (PlayerEntity player : mc.world.getPlayers().stream().sorted(Comparator.comparing(p -> -mc.player.distanceTo(p))).toList()) {
            ...
            int width = Bllry.FONT_MANAGER.getWidth(text);

            if (border.getValue().equalsIgnoreCase("Fill") || border.getValue().equalsIgnoreCase("Both")) Renderer2D.renderQuad(matrices, -width / 2.0f - 1, -Bllry.FONT_MANAGER.getHeight() - 1, width / 2.0f + 2, 0, fillColor.getColor());
            if (border.getValue().equalsIgnoreCase("Outline") || border.getValue().equalsIgnoreCase("Both")) Renderer2D.renderOutline(matrices, -width / 2.0f - 1, -Bllry.FONT_MANAGER.getHeight() - 1, width / 2.0f + 2, 0, outlineColor.getColor());

            Bllry.FONT_MANAGER.drawTextWithShadow(matrices, text, -width / 2, -Bllry.FONT_MANAGER.getHeight(), vertexConsumers, Bllry.MODULE_MANAGER.getModule(FakePlayerModule.class).isToggled() && Bllry.MODULE_MANAGER.getModule(FakePlayerModule.class).getPlayer() == player ? new Color(225, 0, 70) : player.isSneaking() ? new Color(255, 170, 0) : Bllry.FRIEND_MANAGER.contains(player.getName().getString()) ? Bllry.FRIEND_MANAGER.getDefaultFriendColor() : Color.WHITE);
            ...
            if (enchantments.getValue()) {
                for (int i = 0; i < 6; i++) {
                    ...
                    if (!component.getEnchantments().isEmpty()) {
                        int height = (component.getEnchantments().size() * Bllry.FONT_MANAGER.getHeight() / 2) - 18;
            ...
            for (int i = 0; i < 6; i++) {
                ...
                int stackY = -Bllry.FONT_MANAGER.getHeight() - 1 - (items.getValue() ? 18 + maxEnchants : 1);
                ...
                if (items.getValue()) {
                    ...
                    if (stack.getItem().equals(Items.ENCHANTED_GOLDEN_APPLE)) {
                        ...
                        Bllry.FONT_MANAGER.drawTextWithShadow(matrices, "God", 0, 0, vertexConsumers, new Color(255, 125, 255));
                        matrices.pop();
                    }
                    ...
                    if (stack.getCount() != 1) {
                        ...
                        matrices.push();
                        matrices.translate(stackX + 17 - Bllry.FONT_MANAGER.getWidth(count), stackY + 9, 0);
                        Bllry.FONT_MANAGER.drawTextWithShadow(matrices, count, 0, 0, vertexConsumers, Color.WHITE);
                        matrices.pop();
                    }
                }
                ...
                if (durability.getValue() && stack.isDamageable()) {
                    ...
                    matrices.push();
                    matrices.translate(stackX, stackY - Bllry.FONT_MANAGER.getHeight() / 2f - 1, 0);
                    matrices.scale(0.5f, 0.5f, 1);
                    Bllry.FONT_MANAGER.drawTextWithShadow(matrices, Math.round(((stack.getMaxDamage() - stack.getDamage()) * 100.0f) / stack.getMaxDamage()) + "%", 0, 0, vertexConsumers, new Color(red, green, 0));
                    matrices.pop();
                ...
                if (items.getValue() && enchantments.getValue() && stack.hasEnchantments()) {
                    ...
                    for (Object2IntMap.Entry<RegistryEntry<Enchantment>> entry : Object2IntMaps.fastIterable(enchantments)) {
                        ...
                        matrices.scale(0.5f, 0.5f, 1);
                        Bllry.FONT_MANAGER.drawTextWithShadow(matrices, str, 0, 0, vertexConsumers, Color.WHITE);
                        matrices.pop();

                        height += Bllry.FONT_MANAGER.getHeight() / 2;
                    }
                }
            }
            ...
            if (itemName.getValue() && !player.getMainHandStack().isEmpty()) {
                ...
                matrices.push();
                matrices.translate(-Bllry.FONT_MANAGER.getWidth(itemText) / 2f / 2f, -Bllry.FONT_MANAGER.getHeight() - 1 - Bllry.FONT_MANAGER.getHeight() / 2f - 1 - (renderedItems ? (items.getValue() ? 18 + maxEnchants : 1) + (durability.getValue() && renderedDurability ? Bllry.FONT_MANAGER.getHeight() / 2.0f + 1 : 0) : 0), 0);
                matrices.scale(0.5f, 0.5f, 1);
                Bllry.FONT_MANAGER.drawTextWithShadow(matrices, itemText, 0, 0, vertexConsumers, Color.WHITE);
                matrices.pop();
            }
