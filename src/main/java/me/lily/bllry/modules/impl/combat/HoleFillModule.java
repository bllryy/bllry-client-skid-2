...
@RegisterModule(name = "HoleFill", description = "Automatically places blocks inside of holes to prevent others from getting inside of them.", category = Module.Category.COMBAT)
        ...
        Runnable runnable = () -> {
            ...
            if (autoSwitch.getValue().equalsIgnoreCase("None") && flag) {
                if (itemDisable.getValue()) {
                    Bllry.CHAT_MANAGER.tagged("You are currently not holding any " + (webs.getValue() ? "cobwebs." : "blocks."), getName());
                    setToggled(false);
                }
            ...
            if (slot == -1) {
                if (itemDisable.getValue()) {
                    Bllry.CHAT_MANAGER.tagged("No " + (webs.getValue() ? "cobwebs" : "blocks") + " could be found in your hotbar.", getName());
                    setToggled(false);
                }
