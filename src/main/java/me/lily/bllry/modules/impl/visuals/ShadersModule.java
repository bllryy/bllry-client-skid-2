...
public class ShadersModule extends Module {
    ...
    public void onRenderEntity(RenderEntityEvent event) {
        ...
        Color color = (event.getEntity() instanceof PlayerEntity player && Bllry.FRIEND_MANAGER.contains(player.getName().getString()) && !friends.getValue().equals("Sync")) ? (friends.getValue().equals("Default") ? Bllry.FRIEND_MANAGER.getDefaultFriendColor() : friendColor.getColor()) : this.color.getColor();
        event.setVertexConsumers(Bllry.SHADER_MANAGER.create(event.getVertexConsumers(), color));
    }
    ...
    public void onRenderEntity$POST(RenderEntityEvent.Post event) {
        ...
        if (players.getValue() || hostiles.getValue() || animals.getValue() || ambient.getValue() || invisibles.getValue() || items.getValue() || crystals.getValue() || others.getValue()) {
            Bllry.SHADER_MANAGER.getVertexConsumerProvider().draw();
        }

        if (!hands.getValue()) Bllry.SHADER_MANAGER.render(mode.getValue().equalsIgnoreCase("Fill") ? 0 : mode.getValue().equalsIgnoreCase("Outline") ? 1 : 2, opacity.getValue().intValue() / 255.0f);
    }
    ...
    public void onRenderHand(RenderHandEvent event) {
        ...
        event.setVertexConsumers(Bllry.SHADER_MANAGER.create(event.getVertexConsumers(), color.getColor()));
    }
    ...
    public void onRenderShader$POST(RenderShaderEvent.Post event) {
        ...
        Bllry.SHADER_MANAGER.render(mode.getValue().equalsIgnoreCase("Fill") ? 0 : mode.getValue().equalsIgnoreCase("Outline") ? 1 : 2, opacity.getValue().intValue() / 255.0f);
    }
