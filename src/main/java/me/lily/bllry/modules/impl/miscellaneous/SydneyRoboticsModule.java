...
import me.lily.bllry.mixins.accessors.MouseAccessor;
import me.lily.bllry.modules.Module;
import me.lily.bllry.modules.impl.movement.HitboxDesyncModule;
import me.lily.bllry.modules.impl.movement.HoleSnapModule;
...
public class SydneyRoboticsModule extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        try {
            ...
            Bllry.LOGGER.error("An exception has been thrown by clientside Bllry Robotics!", exception);
        }
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (Bllry.MODULE_MANAGER.getModule(HoleSnapModule.class).isToggled()) return;
        if (Bllry.MODULE_MANAGER.getModule(HitboxDesyncModule.class).isToggled()) return;
    ...
    public void onEnable() {
        if (side.getValue().equalsIgnoreCase("Client")) {
            try {
                ...
                Bllry.LOGGER.error("An exception has been thrown by clientside Bllry Robotics!", exception);
                Bllry.CHAT_MANAGER.error("Failed to establish a connection to the SydneyRobotics server.");
                setToggled(false);
            }
        } else {
            try {
                ...
                Bllry.LOGGER.error("An exception has been thrown by serverside Bllry Robotics!", exception);
                Bllry.CHAT_MANAGER.error("Failed to start the SydneyRobotics server.");
                setToggled(false);
            }
        }
    }
        ...
        public void startConnection(int port) throws IOException {
            ...
            while ((inputLine = in.readLine()) != null) {
                ...
                if (split[0].equalsIgnoreCase("module") && split.length == 3) {
                    Module module = Bllry.MODULE_MANAGER.getModule(split[1]);
                    if (module != null) {
                        Bllry.TASK_MANAGER.submit(() -> module.setToggled(Boolean.parseBoolean(split[2])));
                    }
                }

                if (split[0].equalsIgnoreCase("update") && split.length == 2) {
                    Bllry.MODULE_MANAGER.getModule(SydneyRoboticsModule.class).setTarget(split[1]);
                }

                if (split[0].equalsIgnoreCase("key") && split.length == 5 && mc.keyboard != null) {
                    Bllry.TASK_MANAGER.submit(() -> mc.keyboard.onKey(mc.getWindow().getHandle(), Integer.parseInt(split[1]), Integer.parseInt(split[2]), Integer.parseInt(split[3]), Integer.parseInt(split[4])));
                }

                if (split[0].equalsIgnoreCase("mouse") && split.length == 4 && mc.mouse != null) {
                    Bllry.TASK_MANAGER.submit(() -> ((MouseAccessor) mc.mouse).invokeOnMouseButton(mc.getWindow().getHandle(), Integer.parseInt(split[1]), Integer.parseInt(split[2]), Integer.parseInt(split[3])));
                }
                ...
                if (!Bllry.MODULE_MANAGER.getModule(SydneyRoboticsModule.class).isToggled()) {
                    stopConnection();
                    break;
                }
            }
